...
@Injectable()
export class RetentionPolicyAuthorizer implements CustomAuthorizer<any> {
  ...
  async authorize(
    ...
    if (
      ...
      !operator.role?.permissions.includes(
        OperatorPermission.DocumentRetention_View,
      )
    ) {
    ...
    if (
      !authorizerContext.readonly &&
      !operator.role?.permissions.includes(
        OperatorPermission.DocumentRetention_Edit,
      )
    ) {
      throw new UnauthorizedException();
    }
