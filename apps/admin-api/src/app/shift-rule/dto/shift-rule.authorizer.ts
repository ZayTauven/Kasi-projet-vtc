...
@Injectable()
export class ShiftRuleAuthorizer implements CustomAuthorizer<any> {
  ...
  async authorize(
    ...
    if (
      ...
      !operator.role?.permissions.includes(OperatorPermission.ShiftRules_View)
    ) {
      throw new UnauthorizedException();
    }
    if (
      !authorizerContext.readonly &&
      !operator.role?.permissions.includes(OperatorPermission.ShiftRules_Edit)
    ) {
      throw new UnauthorizedException();
    }
    return undefined;
