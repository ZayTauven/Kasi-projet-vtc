...
import 'package:kasi_rider/location_selection/reservation_messages/looking_sheet_view.dart';
...
import 'package:kasi_rider/main/map_providers/google_map_provider.dart';
import 'package:kasi_rider/main/map_providers/open_street_map_provider.dart';
import 'package:kasi_rider/main/order.graphql.dart';
import 'package:kasi_rider/main/pay_for_ride_sheet_view.dart';
import 'package:kasi_rider/main/rate_ride_sheet_view.dart';
        ...
        child: Drawer(
          ...
          child: BlocBuilder<RiderProfileCubit, Query$GetCurrentOrder$rider?>(
              builder: (context, state) => state == null
                  ? DrawerLoggedOut(scaffoldKey: scaffoldKey)
                  : DrawerLoggedIn(
                      rider: state,
      ...
      body: Stack(children: [
        ValueListenableBuilder<Box>(
            ...
            builder: (context, value, child) {
              String? provider = value.get('mapProvider');
              provider ??= mapProvider == MapProvider.googleMap
                  ? 'googlemap'
                  : (mapProvider == MapProvider.mapBox
                      ? 'mapbox'
                      : 'openstreet');
              if (provider == 'mapbox' || provider == 'openstreet') {
                      ...
                      options: Options$Query$GetCurrentOrder(
                          ...
                          onComplete: (result, parsedData) {
                            if (parsedData != null) {
                              ...
                              context
                                  .read<LastOrdersCubit>()
                                  .setLastOrders(parsedData.rider.lastOrders);
