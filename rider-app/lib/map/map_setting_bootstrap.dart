...
/// avant tout login (l'app rider ouvre sur une carte). Ne pas dépendre de la box
/// Hive `user`, qui n'est pas nécessairement ouverte à ce moment-là.
...
Future<bool> _tryFetchMapSetting() async {
  ...
  try {
    final client = GraphQLClient(
      ...
      // longtemps (voir main/graphql_provider.dart). Or cette requete part
      // pendant le demarrage, quand le thread principal est sature : le delai
      // expirait systematiquement, ce qui produisait a la fois un
      // « [BOOTSTRAP CARTE] echec ... TimeoutException after 0:00:05 » trompeur
      // (le reseau etait sain) et, quand la reponse arrivait juste apres, un
