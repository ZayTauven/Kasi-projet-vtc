...
void main() async {
  ...
  // reste de l'app ci-dessous (`Hive.openBox('user')`/`'settings'`) : ce
  // sont deux paquets pub.dev distincts (`hive` vs `hive_ce`), chacun avec
  // son propre etat global. Sans l'appel explicite `Hive.initFlutter()`
  // (extension de `hive_flutter`), le `Hive` de l'app n'est JAMAIS initialise
  // -> `HiveError: You need to initialize Hive or provide a path to store
  // the box` de facon systematique sur `Hive.openBox`. Bug preexistant
  // (aucun rapport avec les paliers de bump de ce chantier), jamais detecte
  // avant faute de test reel sur device/emulateur.
